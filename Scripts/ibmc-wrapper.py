...
def get_command_line(strat, prop, arch, benchmark, concurrency, dargs):
  ...
  command_line = script_ibmc_bb + esbmc_path + vampire_path_new + dargs
...
def verify(strat, prop, concurrency, dargs):
  ...
  if not os.path.exists(c_benchmarks):
    os.makedirs(c_benchmarks)

  shutil.copy(benchmark, c_benchmarks)

  subprocess.run(["./blackbox_llama.py"])

  for file in os.listdir(c_benchmarks):
      os.remove(os.path.join(c_benchmarks, file))
